...
# Loads a video or camera and finds the pupil in each frame
def process_video(video_path, input_method):

    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec for MP4 format
    # Save output next to the script instead of a hardcoded folder
    ...
    if input_method == 2:
        ...
        # Camera mode: input_method is the camera index (0,1,2,…)
        cam_index = video_path
        cap = cv2.VideoCapture(cam_index, cv2.CAP_DSHOW)  # Camera input
        if not cap.isOpened():
            print(f"Error: Could not open camera at index {cam_index}.")
            return
        # Optional: adjust exposure if your camera supports it
    ...
    while True:
        ...
        # Combine the three thresholded images and fit ellipse
...
def select_video():
    ...
    choice = input("Type 'c' for camera or 'v' for video file: ").strip().lower()

    if choice == 'c':
        camera_index = int(input("Enter camera index (0,1,2...): "))
        print(f"Using camera index {camera_index}")
        process_video(camera_index, 1)
        return
